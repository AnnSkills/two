...
class UserServiceIT {
    ...
    void assertThatUserMustExistToResetPassword() {
        userRepository.saveAndFlush(user);
        Optional<UserAnnKh> maybeUser = userService.requestPasswordReset("invalid.login@localhost");
    ...
    void assertThatOnlyActivatedUserCanRequestPasswordReset() {
        ...
        Optional<UserAnnKh> maybeUser = userService.requestPasswordReset(user.getLogin());
        assertThat(maybeUser).isNotPresent();
    ...
    void assertThatResetKeyMustNotBeOlderThan24Hours() {
        ...
        Optional<UserAnnKh> maybeUser = userService.completePasswordReset("johndoe2", user.getResetKey());
        assertThat(maybeUser).isNotPresent();
        userRepository.delete(user);
    }
    ...
    void assertThatNotActivatedUsersWithNotNullActivationKeyCreatedBefore3DaysAreDeleted() {
        ...
        UserAnnKh dbUser = userRepository.saveAndFlush(user);
        dbUser.setCreatedDate(now.minus(4, ChronoUnit.DAYS));
        userRepository.saveAndFlush(user);
        Instant threeDaysAgo = now.minus(3, ChronoUnit.DAYS);
        List<UserAnnKh> users = userRepository.findAllByActivatedIsFalseAndActivationKeyIsNotNullAndCreatedDateBefore(threeDaysAgo);
    ...
    void assertThatNotActivatedUsersWithNullActivationKeyCreatedBefore3DaysAreNotDeleted() {
        ...
        assertThat(users).isEmpty();
        userService.removeNotActivatedUsers();
        Optional<UserAnnKh> maybeDbUser = userRepository.findById(dbUser.getId());
        assertThat(maybeDbUser).contains(dbUser);
    }

    private void generateUserToken(UserAnnKh user, String tokenSeries, LocalDate localDate) {

...
describe('Task e2e test', () => {
  ...
  const taskSample = { taskName: 'кортеж' };
  ...
  describe('new Task page', () => {
    ...
    it('should create an instance of Task', () => {
      cy.get(`[data-cy="taskName"]`).type('expedite').should('have.value', 'expedite');

      cy.get(`[data-cy="description"]`).type('область Нижегородская координатор').should('have.value', 'область Нижегородская координатор');

      cy.get(`[data-cy="deadline"]`).type('2022-05-01').should('have.value', '2022-05-01');
